...
metrics. This is the backend a console UI is built against (a React/TS
console is planned but out of scope for this pass — every endpoint below
already works and is covered by `tests/test_console_api.py`).
...
API types to drift from the backend). Three pages:
...
| `test_console_api.py` | 25 | Policy CRUD, dry-run, live-reload, audit filters + cursor pagination, stats |

Local count with the console API tests added is 278 (Python 3.12, semantic
detector disabled). The badge above stays at 244 until that's confirmed
green on CI's actual environment (Python 3.11, real Postgres) rather than
copied from a local run.
